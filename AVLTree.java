 ...
 class LUC_AVLTree {
     ...
     private Node deleteElement(int value, Node node) {
         ...
         if (node == null) {
            ...
            //#2
         if (node.leftChild != null && value < node.value) {               //if node's left != null && value less than node's value, delete left
            node.leftChild = deleteElement(value, node.leftChild);
            
         } else if (node.rightChild != null && value > node.value) {       //if node's right != null && value greater than node's value, delete right
            node.rightChild = deleteElement(value, node.rightChild);

         } else {                                                           //node.leftChild == null && node.rightChild == null (leaf node) 
            if (value != node.value) {                                      //and value not found
                return node;
            } else {                                                        //value == node.value -> value has been found
/* 
                if (node.leftChild == null && node.rightChild == null) {    //if node has no subchildren, return null
                    return null;
                } else if ((node.leftChild != null && node.rightChild == null)) { //if node has left but no right child, move left subtree up
                    node = node.leftChild;
                    return node;         
                } else if ((node.leftChild == null && node.rightChild != null)) { //if node has right but no left child, move right subtree up
...
*/ 
                if (node.leftChild == null && node.rightChild == null) {    //if node has no subchildren, return null
                    ...
                    return node.leftChild;
                                                                     ...
                                                                     //left && right subtrees (hopefully)
                Node success = minValueNode(node.rightChild);               //in-order successor - smallest val in right subtree
                node.value = success.value;                                 //assign successor node val to this node val
                node.rightChild = deleteElement(success.value, node.rightChild);        //delete successor node
            }
                

            node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild));      //here?
            int bf = getBalanceFactor(node);

            //if (bf > 1) {
            if (Math.abs(bf) > 1) {                                     //if node's |bf| > 1 -> left heavy
                int leftBF = getBalanceFactor(node.leftChild);          //node.leftChild's bf
                //if (value < node.leftChild.value) {
                if (leftBF >= 0) {       //> 0                               //if node.leftChild's bf > 0, left heavy, perform LL; else LR
                    //node = LLRotation(node.leftChild);    //LL Case
                    //node = LRRotation(node.leftChild);      //LR Case
                    //return node;
                    return LLRotation(node);
                } else {
                    //node = LRRotation(node.leftChild);    //LL Case
                    //node = LLRotation(node.leftChild);      //LR Case
                    //return node;
...
/* */     } else if (Math.abs(bf) < 1) {                
                ...
                if (rightBF <= 0) {             // < 0
                    node = RRRotation(node);
                    //return node;
                    //return RRRotation(node);       //this one
                    //return RLRotation(node);
                    return node;
                    
                } else {
                    //node = RLRotation(node.rightChild);
                    //return node;
                    return RLRotation(node);       
